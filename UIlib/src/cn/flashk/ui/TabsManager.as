package cn.flashk.ui
{
	...
	public class TabsManager extends EventDispatcher
	{
		...
		protected var _textUpColor:uint = 0xFFFFFF;
		protected var _textOverColor:uint = 0xFF0000;
		protected var _textDownCloor:uint = 0x00FF00;
		
		protected var _isRemoveChild:Boolean = false;
		protected var _all:Array;
		protected var _allState:Array;
		protected var _txts:Vector.<TextField>;
		...
		protected function onTabBtnRollOut(event:MouseEvent):void
		{
			...
			if(index<_txts.length){
				_txts[index].textColor = textUpColor;
			}
		}
		...
		protected function onTabBtnRollOver(event:MouseEvent):void
		{
			...
			var index:uint = getSimpleButtonIndex(btn);
			if(index<_txts.length){
				_txts[index].textColor = textOverColor;
			}
		}
		
		protected function onTabBtnRollDown(btn:SimpleButton):void
		{
			if(_txts == null) return;
			var index:uint = getSimpleButtonIndex(btn);
			if(index<_txts.length){
				_txts[index].textColor = textDownCloor;
			}
		}
		...
		public function switchToTab(index:uint):void
		{
			...
			if((_nowIndex-1)<_txts.length && _nowIndex != 0){
				_txts[_nowIndex-1].textColor = textUpColor;
			}
			_nowIndex = index;
