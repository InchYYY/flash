package cn.flashk.ui
{
	...
	public class Xdis
	{
		...
		public static function setCenterPos(target:DisplayObject,posX:int=0,posY:int=0):void
		{
			target.x = -int(target.width/2)+posX;
			target.y = -int(target.height/2)+posY;
		}
		
		public static function setCenterPosX(target:DisplayObject,posX:int=0):void
		{
			target.x = -int(target.width/2)+posX;
		}
		
		public static function setCenterPosY(target:DisplayObject,posY:int=0):void
		{
			target.y = -int(target.height/2)+posY;
		}
		
		public static function stopMovieClips(... args):void
		{
		...
		public static function goToStopMovieClips(stopAt:Object,... args):void
		{
			var len:int = args.length;
			for(var i:int=0;i<len;i++){
				var mc:MovieClip = args[i] as MovieClip;
				if(mc) mc.gotoAndStop(stopAt);
			}
		}
