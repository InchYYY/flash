package cn.flashk.controls
{
	...
	public class List extends UIComponent
	{
		...
		{
			var arr:Array = [];
			var selectItemBak:Object = selectedItem;
			var num:int = items.numChildren;
			for(var i:int = 0;i<num;i++){
				arr.push(getItemAt(i));
			}
			arr.sortOn(fieldName,options);
			for(i=0;i<num;i++){
				items.setChildIndex(DisplayObject(findRender(arr[i])),i);
			}
			if(selectItemBak != null){
				selectedItem = selectItemBak;
			}
			reAlign(null);
		}
		 ...
		 * 
		 */
		public function findRender(item:Object):DisplayObject
		{
			var num:int = items.numChildren;
			for(var i:int=0;i<num;i++){
				var render:Object = Object(items.getChildAt(i));
				if(render.data == item){
					return render as DisplayObject;
				}
			}
			return null;
		}
