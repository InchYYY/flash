package cn.flashk.controls
{
	...
	import cn.flashk.controls.events.UIEvent;
	import cn.flashk.controls.interfaces.IListRender;
	import cn.flashk.controls.interfaces.ITreeRender;
	import cn.flashk.controls.layout.Align;
	import cn.flashk.controls.support.TreeNodeRenderBase;
	import cn.flashk.controls.support.TreeSprite;
	import cn.flashk.controls.support.TreeTrunkRenderBase;
	...
	public class DoubleDeckTree extends UIComponent
	{
		...
		public static var scrollRectLess:int = 0;
...
//		protected var _ui:Sprite;
		...
		public function DoubleDeckTree()
		{
			...
			_scrollBar.setTarget(_renderContainer,false,_compoWidth,_compoHeight);
			_compoWidth = 200;
			_compoHeight = 300;
		...
		public function openTrunk(index:int,isOpen:Boolean = true):void
		{
			if(index<0) return;
		...
		public function addTrunkAt(data:Object,index:int):void
		{
			...
			UIComponent.stage.addEventListener(Event.RENDER,onStageRender);
		}
		...
		public function clearTrunkAllChilds(trunkIndex:int):void
		{
			...
			while(sp.numChildren>1){
				if(_lastSelect == sp.getChildAt(0+1)){
					_lastSelect = null;
				}
				sp.removeChildAt(0+1);
			}
			updateOneTree(sp);
			sp.update();
			UIComponent.stage.invalidate();
...
//			this.addChildAt(_ui,0);
		...
		{
			UIComponent.stage.removeEventListener(Event.RENDER,onStageRender);
