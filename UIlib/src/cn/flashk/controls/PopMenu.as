package cn.flashk.controls
{
	...
	import cn.flashk.controls.managers.ButtonTextStyle;
	...
	public class PopMenu extends UIComponent
	{
		...
		public static var defaultMenuSpaceLinkage:String = "uiSkin.menuSpace1";
		
		public static var textMaxWidth:int = 300;
		public static var nameIcon:String = "icon_image";
		public static var nameText:String = "label_txt";
		public static var nameMouseEffect:String = "mouseEffect_mc";
		
		public static var popAtParent:DisplayObjectContainer;
		...
		public function PopMenu()
		{
			...
			_compoWidth = 0;
			_compoHeight = 0;
			setMenuSkin(PopMenu.defaultBagckGroundLinkage,PopMenu.defaultMenuItemLinkage);
		}
		...
		public function setMenuSkin(bagckGroundLinkage:String,menuItemLinkage:String,textColor:ButtonTextStyle=null):void
		{
			_textColor = textColor;
			if(_textColor == null){
				_textColor = new ButtonTextStyle();
				_textColor.outColor = 0xFFFFFF;
				_textColor.overColor = 0xFFFF00;
				_textColor.downColor = 0xCCCCCC;
			}
		...
		{
			return ObjectFactory.getNewSprite(defaultMenuSpaceLinkage);
		}
		...
		public function initByArray(arr:Array,itemClickCallFunction:Function):void
		{
			for(var i:int=0;i<arr.length;i++){
				if(arr[i] is String){
					addItem(arr[i],itemClickCallFunction);
				}
		 ...
		 * 
		 */
		public function addSpace(spaceDisplay:DisplayObject=null):void
		{
			if(spaceDisplay == null){
				spaceDisplay = creatDefautSpace();
			}
		...
		{
			var item:Sprite = ObjectFactory.getNewSprite(_menuItemLinkage);
			...
			var icon:Image = item.getChildByName(nameIcon) as Image;
			if(icon){
		...
		{
			if(_bg == null){
				_bg = ObjectFactory.getNewSprite(_bagckGroundLinkage);
				this.addChildAt(_bg,0);
			}
			initSize();
			if(popAtParent != null){
				popAtParent.addChild(this);
			}else{
			...
			if(sx < 0) sx = 0;
			if(sy < 0) sy = 0;
			this.x = int(sx*UISet.stageScale);
			this.y = int(sy*UISet.stageScale);
			_scrollH = 0;
			this.scrollRect = new Rectangle(0,0,0,_scrollH);
			this.alpha = 0;
			this.addEventListener(Event.ENTER_FRAME,onEnterFrameAdd);
		}
		
		protected function onEnterFrameAdd(event:Event):void
		{
			_scrollH += (_compoHeight+5-_scrollH)/3;
			this.alpha = _scrollH/(_compoHeight+5);
			if(Math.abs(_compoHeight+5-_scrollH) < 1.1){
				_scrollH = _compoHeight+5;
				this.alpha = 1;
				this.removeEventListener(Event.ENTER_FRAME,onEnterFrameAdd);
			}
