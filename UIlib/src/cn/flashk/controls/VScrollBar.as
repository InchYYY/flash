package cn.flashk.controls 
{
	...
	public class VScrollBar extends UIComponent
	{
		 ...
		 * 默认的连续按下上下箭头的滚动速度 
		 */
		public static var defaultArrowFrameMoveSpeed:Number = 4;
		/**
		 * 滑进时的透明度变化速度 
		 */
		...
		public static var mouseMoveStick:uint = 6;
		public static var isDragUpSmothMove:Boolean = false;
		
		public var mobileMA:Number = 6;
		public var mobileMB:Number = 1.5;
		public var mobileMC:Number = 2.9;
		public var mobileMD:Number = 5;
		
		private var _moveSpeedCount:int;
		private var _moveAddCountAdd:Number;
		private var _lastScreenAllMouseYs:Array = [];
		private var _lastScreenMouseY:int=0;
		private var _moveSpeed:Number;
		private var _moveSppedStartY:Number;
...
//			newSize += 2;
		...
		protected function startDragScroller(event:MouseEvent):void 
		{
			...
			_isMouseDrag = true;
			unableUpdate = true;
			this.removeEventListener(Event.ENTER_FRAME,onMoveSpeedEnterFrame);
		...
		protected function startHandDrag(event:MouseEvent):void 
		{
			...
			handY = getMousePos();
			handRecY = getRectPos();
			UI.stage.mouseChildren = UISet.dragStageMouseAble;
			_lastScreenAllMouseYs = [];
			this.stage.addEventListener(MouseEvent.MOUSE_MOVE, handDrag);
			this.stage.addEventListener(MouseEvent.MOUSE_UP, stopHandDrag);
			this.stage.addEventListener(Event.ENTER_FRAME,recodeMouseY);
		}
		
		protected function recodeMouseY(event:Event):void
		{
		...
		protected function stopHandDrag(event:MouseEvent):void 
		{
			...
			this.stage.removeEventListener(Event.ENTER_FRAME,recodeMouseY);
			UI.stage.mouseChildren = true;
			if(isDragUpSmothMove == true){
				if(_lastScreenAllMouseYs.length >mouseMoveStick){
					_lastScreenMouseY = _lastScreenAllMouseYs[_lastScreenAllMouseYs.length-mouseMoveStick];
				}else if(_lastScreenAllMouseYs.length >1){
					_lastScreenMouseY = _lastScreenAllMouseYs[0];
				}else{
					_lastScreenMouseY = UIComponent.stage.mouseY;
				}
				_moveSpeed = -(UIComponent.stage.mouseY-_lastScreenMouseY)*UISet.stageScale/mobileMA;
				if(Math.abs(_moveSpeed)>1){
					_moveAddCountAdd = 65/mobileMB/Math.abs(_moveSpeed);
					if(_moveAddCountAdd <mobileMC){
						_moveAddCountAdd = mobileMC;
					}
					if(_moveAddCountAdd > mobileMD){
						_moveAddCountAdd = mobileMD;
					}
					var hy:Number;
					_lastScreenMouseY = this.mouseY;
					...
					_moveSppedStartY = hy;
					_moveSpeedCount = 0;
					this.addEventListener(Event.ENTER_FRAME,onMoveSpeedEnterFrame);
					this.addEventListener(Event.REMOVED_FROM_STAGE,clearDragEnterfameMove);
				}
			}
		}
		
		public function clearDragEnterfameMove(event:Event = null):void
		{
			this.removeEventListener(Event.ENTER_FRAME,onMoveSpeedEnterFrame);
		}
		
		protected function onMoveSpeedEnterFrame(event:Event):void
		{
			_moveSpeedCount += _moveAddCountAdd;
			if(_moveSpeedCount > 90) {
				_moveSpeedCount = 90;
				this.removeEventListener(Event.ENTER_FRAME,onMoveSpeedEnterFrame);
			}
			var sc:Number = Math.cos(_moveSpeedCount*Math.PI/180);
			if(sc<0){
				this.removeEventListener(Event.ENTER_FRAME,onMoveSpeedEnterFrame);
			}
			_moveSppedStartY += _moveSpeed*sc;
			var hy:Number = _moveSppedStartY;
			if (hy < 0) 
			{
				hy = 0;
				this.removeEventListener(Event.ENTER_FRAME,onMoveSpeedEnterFrame);
		...
		{
			var hy:Number;
			_lastScreenAllMouseYs.push(UIComponent.stage.mouseY);
			hy = handRecY - (getMousePos() - handY);
			updateMaxPos();
			if (hy < 0) 
			{
