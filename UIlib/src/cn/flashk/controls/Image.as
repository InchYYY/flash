package cn.flashk.controls
{
	...
	import cn.flashk.utils.FileManager;
	...
	public class Image extends UIComponent
	{
		...
		public var mobileIsLocal:Boolean = true;
		
		public var checkExit:Boolean = false;
		...
		public function set linkage(value:String):void
		{
			...
			if(value == "") return;
			var obj:Object = ObjectFactory.getObject(value);
			var bd:BitmapData = obj as BitmapData;
			if(bd){
				_disObj = new Bitmap(bd);
				Bitmap(_disObj).smoothing = smoothing;
			}else{
		...
		{
			if(UIComponent.isMobile == true) return;
			setSkin(ImageSkin)
		}
		...
		{
			if(UIComponent.isMobile == false || mobileIsLocal == false){
	            if(ldr == null)
	            {
				    ldr = new Loader();
				...
				_file = FileManager.getAppFile(filePath);
				if(checkExit == false || _file.exists){
					if(!_fs){
						_fs = new FileStream();
					}
					_fs.addEventListener(Event.COMPLETE,onFileStreamOpenComplete);
					_fs.openAsync(_file,FileMode.READ);
				}
			}
		}
		
		protected function onFileStreamOpenComplete(event:Event):void
		{
