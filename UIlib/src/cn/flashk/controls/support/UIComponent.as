package cn.flashk.controls.support 
{
	...
	public class UIComponent extends Sprite
	{
		...
		public static var popParent:DisplayObjectContainer;
       
        private static var _isInited:Boolean = false;
		
		public static function get popUpAddParent():DisplayObjectContainer
		{
			if(popParent != null) return popParent;
			return stage;
		}
		...
		protected var _isSetSizeNeedDraw:Boolean = false;
		...
		public function showTipNow():void
		{
			if(stage){
				ToolTip.showAToolTipOnStage(stage,toolTip);
				stage.addEventListener(MouseEvent.MOUSE_UP,onStageTipMouseUp);
			}
		}
		
		protected function onStageTipMouseUp(event:MouseEvent):void
		{
			if(ToolTip.toolTipInstance && ToolTip.toolTipInstance.parent){
				ToolTip.toolTipInstance.parent.removeChild(ToolTip.toolTipInstance);
			}
			if(stage){
				stage.removeEventListener(MouseEvent.MOUSE_UP,onStageTipMouseUp);
			}
		}
		...
		public function setSize(newWidth:Number, newHeight:Number):void 
		{
			...
			if(this.stage){
				...
				this.addEventListener(Event.ADDED_TO_STAGE,onAddToStageRedraw);
			}
			this.dispatchEvent(new UIComponentEvent(UIComponentEvent.RESIZE));
		}
		
		protected function onAddToStageRedraw(event:Event):void
		{
			this.removeEventListener(Event.ADDED_TO_STAGE,onAddToStageRedraw);
			if (skin != null) {
				try{
					skin.reDraw();
				}catch (e:Error) {
