package  debug
{
    ...
    public class Debug
    {
        private static var bpause:Boolean = false;
        /**
         * Deubg文字框的宽度
         */
         ...
         * Debug 点击小的设定高度
         */
        private static var height:int = 300;
        /**
         * 默认是否打开Firefox的Firebug和Chrome的控制台输出
         */
        private static var browserConsoleIsOn:Boolean = false;
        /**
         * 常用颜色，可以用在Debug.traceColor参数中
         */
        private static var colors:Array = [0xFF9999 , 0x99CC99 , 0x009999];
        /**
         * trace和traceObject的文本颜色
         */
        private static var colorTrace:uint = 0xbcbcbc;
        /**
         * erro的文本颜色
         */
        private static var colorError:uint = 0xe22622;
        /**
         * log的文本颜色
         */
        private static var colorLog:uint = 0x119c11;
        /**
         * warn的文本颜色
         */
        private static var colorWarn:uint = 0xe2c43c;
        /**
         * 时间显示的颜色
         */
        private static var colorTime:uint = 0x766060;
        /**
         * 致命错误的颜色
         */
        private static var colorFatal:uint = 0xFF3300;
        /**
         * state的文本颜色
         */
        private static var colorState:uint = 0xe46a81;
        /**
         *  recordUserOperate的文本颜色
         */
        private static var colorUserOperate:uint = 0x387edd;
        /**
         * Debug内存显示的颜色
         */
        private static var colorMem:uint = 0xFFCC00;

        /**
         * 拖动栏和边框颜色
         */
        private static var colorDragLine:uint = 0x55BBFF;
        private static var colorSend:uint = 0xE9D048;
        private static var colorGet:uint = 0xE912B1;
        /**
         * 文本框背景色
         */
        private static var backgroundColor:String = "#000000";
        /**
         * 点击更换背景透明度后背景色
         */
        private static var backgroundChangeColor:String = "#000000";

        /**
         * 文本框背景透明度
         */
        private static var backgroundAlpha:Number = 0.95;
        /**
         * 点击更换背景透明度后文本框背景透明度
         */
        private static var backgroundChangeAlpha:Number = 0.0;
        /**
         * 当前是否在IDE环境中
         */
        private static var isIDE:Boolean = false;

        private static var stage:Stage;
//        /**
//         * 是否记录整个舞台的鼠标事件
//         */
//        private static var isRecordUserOperate:Boolean = false;

        private static var htmlCharset:String = "gb2312";
        /**
         * 按组合快捷键的键位keyCode，默认使用Ctrl+Alt+Y键打开。
         */
        private static var openKeyCode:int = 89;
        /**
         *the miner 测试 
         */        
        private static var OPEN_THE_MINER:int = 90;
        private static var hideTopTextKeyCode:int = 85;
        private static var viewTextTextFormat:TextFormat = new TextFormat ("Verdana,Arial,宋体" , 12 , 0x0);
        private static var isTraceInFlashBuilderIDE:Boolean = true;
        /**
         * 是否记录消息的时间，对于极高的数据量，可以将此值设定为false，可以获得接近30%的性能提升
         */
        private static var isRecordTime:Boolean = true;

        private static var _isView:Boolean = false;
        ...
        private static var btnPause:Sprite;
        private static var btnClear:Sprite;
...
//        private static var isUpdated:Boolean = false;
        ...
        private static function get textChangeFilters():Array
        {
         ...
         *
         */
        private static function set textChangeFilters(value:Array):void
        {
        ...
        private static function get textFilters():Array
        {
            return _textFilters;
        }
         ...
         *
         */
        private static function set textFilters(value:Array):void
        {
            _textFilters = value;
            if (bgMode == 1)
...
//            }
        ...
        protected static function checkFPS(event:Event):void
        {
            ...
            {
                fpsFrameCount = 0;
                if (nowFPS > Debug.stage.frameRate)
                    nowFPS = Debug.stage.frameRate;
                if (nowFPS < 0)
                    nowFPS = 0;
...
//            }
            ...
            dragArea.graphics.drawRect (0 , 0 , width , 15);
            dragArea.graphics.beginFill (colorDragLine , 1);
            dragArea.graphics.drawRect (0 , 0 , width + 1 , 7);
         ...
         *
         */
        private static function get operateString():String
        {
         ...
         *
         */
        private static function get errorString():String
        {
         ...
         *
         */
        private static function get allString():String
        {
         ...
         *
         */
        private static function updateTopText(htmlText:String , index:uint = 0 , color:uint = 0xF000000):void
        {
            if (color != 0xF000000)
            {
         ...
         *
         */
        private static function traceObject(obj:Object , frontString:String = ""):void
        {
         ...
         *
         */
        private static function changeTopTextState(state:String = "show" , textColor:uint = 0x666666 , align:String = "R" , textFormat:TextFormat = null , spaceX:Number = 10 , spaceY:Number = 10 , textWidth:uint = 200 , textHeight:uint = 54):TextField
        {
            if (state == "show")
            {
                if (Debug.stage != null)
         ...
         *
         */
        private static function traceWithColor(color:uint , ... args):void
        {
         ...
         *
         */
        private static function warn(... args):void
        {
            writeIn (colorWarn , warnArr , "**warn**" , args);
        }

        public static function send(... args):void
        {
            writeIn (colorSend , logArr , "**log**" , args);
        }

        public static function getmsg(... args):void
        {
            writeIn (colorGet , logArr , "**log**" , args);
        }
        ...
        {
            writeIn (colorLog , logArr , "**log**" , args);
        }
         ...
         *
         */
        private static function state(... args):void
        {
         ...
         *
         */
        private static function recordUserOperate(... args):void
        {
            opeCount++;
            var str:String;
         ...
         *
         */
        private static function traceToConsole(... args):void
        {
         ...
         *
         */
        private static function clearFirebug():void
        {
         ...
         *
         */
        private static function get messageLength():uint
        {
         ...
         *
         */
        private static function saveLog(event:MouseEvent = null):void
        {
        ...
        private static function writeIn(color:uint , writeInArray:Array , consoleStr:String , argsArr:Array , AddFrontString:String = "" , addEndString:String = ""):void
        {
            if (isView == true && bpause == false) //&& isUpdated == false)
            {
                ...
                var conMsg:String;
                if (isIDE == true && isTraceInFlashBuilderIDE == true)
                {
...
//                }
        ...
        private static function formatHTML(isAddSystemInfo:Boolean = false):String
        {
            ...
            {
                var date:Date = new Date ();
                addStr = '<font color="#' + colorWarn.toString (16) + '">============SystemInfo============<br/><br/>' + Capabilities.serverString + '<br/><br/>' + 'flash useMemory:' + Number (System.totalMemory / 1024 / 1024).toFixed (6) + ' mb   IME:' + IME.conversionMode + '<br/><br/>' + date.toString () + '/' + (date.month + 1) + '/' + date.date + '<br/><br/>' + '============SystemInfo============<br/><br/>' + '</font>';
            }
            ...
            var str:String = '<html><head><meta http-equiv="Content-Type" content="text/html; charset=' + htmlCharset + '" /></head><body style="font-family:arial,verdana;font-size:10pt;background-color:' + backgroundColor + '">' + addStr + allMsgStr + '</body></html>';
            return str;
        }
        ...
        private static function onpauseClick(e:MouseEvent):void
        {
            bpause = !bpause;
        }

        private static function onClearClick(e:MouseEvent):void
        {
            allMsg = [];
            logArr = [];
            operateArr = [];
        ...
        private static function checkKey(event:KeyboardEvent):void
        {
            if (event.ctrlKey == true && event.altKey == true)
            {
                ...
                if(event.keyCode == OPEN_THE_MINER)
                {
                   // LoadModuleManager.getInstance().callModuleFunction("TheMinerTest",true,"init",null);
                }
            }
        }
