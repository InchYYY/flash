package nl.bron
{
	...
	dynamic public class brContainer extends MovieClip
	{
		...
		public static var ED:*;
		public static var MV:*;
		...
		{
			dispatchEvent(new Event("STOP_FLV"));
		}

		public function playFlv():void
		{
			dispatchEvent(new Event("PLAY_FLV"));
		}
		...
		public function loadDefaultImage(asLink:String):BitmapData
		{
			...
			if (!this.loaderInfo)
				return null;
			if (!this.loaderInfo.applicationDomain.hasDefinition(asLink))
				return null;
		...
		public function setCell(id:int, paramStr:String):void
		{
			...
			else
			{
				...
				else if (parm.zoom != undefined)
				{
					ptype=(parm.default == "logo") ? "LOGO" : "PIC";
				}
				else
				{
					ptype="HEAD";
			...
			{
				case "TEXT":
					ctype=HostCell.TYPE_TEXT;
					rtype=Resource.RES_TEXT;
					title="文字";
					break;
				case "PIC":
					ctype=HostCell.TYPE_PICT;
					rtype=Resource.RES_PICT;
					title="图片";
					break;
				case "LOGO":
					ctype=HostCell.TYPE_PICT;
				...
				case "BG":
					...
					break;
				case "HEAD":
					ctype=HostCell.TYPE_HEAD;
					rtype=Resource.RES_PICT;
					title="头像";
					break;
				case "MOV":
					ctype=HostCell.TYPE_MOV;
					rtype=Resource.RES_MOV;
					title="视频";
					break;
				default:
					throw new Error("Cell type error: cell_" + id + ", type=" + ptype + "（Allow: TEXT, PIC, LOGO, BG, HEAD）");
			}
			//-----------------------------------------------------------------------------------------------------------------
		...
		public function regCell(id:int, cell:brCell):HostCell
		{
			...
			if (!contains(cell))
				addChild(cell);
			if (hc.actor && !cell.contains(hc.actor))
				cell.addChild(hc.actor);
