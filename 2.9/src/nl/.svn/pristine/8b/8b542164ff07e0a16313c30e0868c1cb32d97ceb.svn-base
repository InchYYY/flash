package nl.util
{
	...
	public class Net
	{
		...
		public static function getImage(url: String, callback: Function): void
		{
			...
			fontLoader.addEventListener(IOErrorEvent.IO_ERROR, function (e: IOErrorEvent) : void {trace("图片获取失败：", url)});
		 ...
		 * 获取一个视频对象(Video)
		 */
		public static function getMov(url: String, callback: Function, err:Function = null, errCount:int = 0): void
		{
			var flvNetStr: NetStream;
			var flvVideo: Video;
			var flvNetCon: NetConnection;
			var flvObject: Object = new Object();
			var isLoadeFLV:Boolean = false;
			...
			trace(url);
			//=================定时器，=================
			var isErr:Boolean = true;
			var tt:Timer = new Timer(3000,1);
			tt.addEventListener(TimerEvent.TIMER_COMPLETE,function(e:TimerEvent):void{
				trace("tt",isErr);
				tt.stop();
				if(isErr){
					isLoadeFLV = true;
					...
					getMov(url, callback, err, errCount)
				}
			});
			tt.start(); 
			//=================播放信息=================
