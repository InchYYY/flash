package nl.bron 
{
	...
	public class brMovCell extends brCell
	{
		...
		{
			super();
			_container.addEventListener(Event.ENTER_FRAME, onEnter);
			trace(_hostCell.ID,this.name);
			...
			_container.addEventListener("STOP_FLV", function(e:Event):void{if(_hostCell.mov)_hostCell.mov.pause();});
			_container.addEventListener("PLAY_FLV", function(e:Event):void{if(_hostCell.mov)_hostCell.mov.resume();});
		}
		...
		private function playMov(e:Event = null): void
		{
			...
			p = (p < 20) ? 0 : p;
			trace(_hostCell.ID,this.name,"play",p/25,_container.currentFrame, parent.name);
			hostCell.mov.seek(p/25);
		...
		{
			if (_playing) {
				trace(_hostCell.ID,this.name,"stop",_hostCell.mov.time*25,_container.currentFrame, parent.name);
				_hostCell.mov.pause();
				_playing = false;
			}
		}
